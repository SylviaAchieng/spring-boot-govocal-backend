...
public class ImageMapperDecorator implements ImageMapper{
    ...
    public ImageDto toDto(Image image) {
        ...
        byte[] imageUrl = image.getImageUrl();
        if (imageUrl!=null && (imageUrl.length>0)){
            image.setImageUrl(Base64.getEncoder().encode(imageUrl));
        }
        return mappedDto;
    }
    ...
    public Image toEntity(ImageDto imageDto) {
        ...
        if (base64EncodedImage!=null && !base64EncodedImage.isEmpty()){
            mappedEntity.setImageUrl(Base64.getDecoder().decode(base64EncodedImage));
        }
        return mappedEntity;
    }
}
