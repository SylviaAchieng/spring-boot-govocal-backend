...
public class ProjectServiceImpl implements ProjectService{
    ...
    public GenericResponseV2<ProjectsDto> createProject(ProjectsDto projectsDto) {
        try {
            Projects projectToBeSaved = projectMapper.toEntity(projectsDto);
            Projects savedProject = projectRepository.save(projectToBeSaved);
    ...
    public GenericResponseV2<ProjectsDto> getProjectById(BigDecimal projectId) {
        try {
            Projects projectFromDb = projectRepository.findByProjectId(projectId).orElseThrow(() -> new RuntimeException("Project not found"));
    ...
    public GenericResponseV2<Boolean> deleteProjectById(BigDecimal projectId) {
        try {
            Projects projectFromDb = projectRepository.findByProjectId(projectId).orElseThrow(() -> new RuntimeException("Issue not found"));
    ...
    public GenericResponseV2<Boolean> updateEventById(ProjectsDto projectsDto, BigDecimal projectId) {
        try {
            Projects projectToSave = projectMapper.toEntity(projectsDto);
            Projects savedProject = projectRepository.save(projectToSave);
